...
def main():
    ...
    for claim in claims_train:
        if not claim.get("cited_doc_ids"):
            continue
        claim_text = QUERY_PREFIX + claim["claim"]
    ...
    for claim in monitor_claims:
        if not claim.get("cited_doc_ids"):
            continue
        qid = str(claim["id"])
        queries[qid] = QUERY_PREFIX + claim["claim"]
    ...
    output_path = "results/arctic_finetuned"

    print("\n____Baseline (pretrained Arctic-embed-l-v2.0)____")
    ...
    best_model = SentenceTransformer(output_path, device="cuda", trust_remote_code=True)
