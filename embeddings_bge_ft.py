...
model = SentenceTransformer("results/bge_finetuned", device="cuda")
...
np.save(results_dir / "corpus_bge_ft_doc_ids.npy", doc_ids)
np.save(results_dir / "corpus_bge_ft_embeddings.npy", embedding)

faiss.normalize_L2(embedding)
d = embedding.shape[1]
index = faiss.IndexFlatIP(d)
index.add(embedding)
faiss.write_index(index, str(results_dir / "faiss_bge_ft_index.bin"))